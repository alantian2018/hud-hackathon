...
class ActorCritic(nn.Module):
    ...
    def __call__(self, observation: Observation):
        ...
        x = nn.Conv(16, (5, 5), strides=(2, 2), padding="SAME")(raster)
        x = nn.relu(x)
        x = nn.Conv(24, (3, 3), strides=(2, 2), padding="SAME")(x)
        x = nn.relu(x)
        x = x.reshape((x.shape[0], -1))
        x = nn.Dense(self.hidden_size)(x)
        x = nn.relu(x)

        s = nn.Dense(self.hidden_size)(structured)
        s = nn.relu(s)
        global_features = nn.Dense(self.hidden_size)(jnp.concatenate([x, s], axis=-1))
